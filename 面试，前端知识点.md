...
解决这种情况的办法 使用this.$set(obj,obj.value,new value)
   也可以使用 Vue.set(obj,obj.value,new value)
...
Vue.js 是一个流行的JavaScript框架，主要用来构建用户界面。单页应用（SPA，Single-page Application）和多页应用（MPA，Multi-page Application）是两种不同的Web应用架构：

单页应用（SPA）：

在SPA中，整个页面只加载一次。浏览器通过前端路由（如Vue Router）、哈希（Hashchange）或HTML5 History API管理URL。当用户导航时，实际上是应用内的内容改变，新内容通过JavaScript异步获取（Ajax/JSONP）并动态渲染到页面上，无刷新。
特点：保持URL不变，页面状态在内存中管理，SEO友好性相对较低，因为搜索引擎通常无法执行JavaScript，可能需要手动做适配。
多页应用（MPA）：

MPA是一组独立的静态HTML页面，每个页面都有自己的服务器响应。用户在浏览器地址栏中输入不同的URL会导致服务器返回不同的HTML。
特点：每个页面都有完整的HTML结构，SEO友好，搜索引擎可以直接抓取内容，自然的导航结构，但页面间的通信需要通过服务器，无法实现即时更新。
Vue.js可以良好地支持这两种应用模式。对SPA，Vue可以通过组件化和路由管理来构建，对于MPA，虽然主要还是创建静态页面，但可以利用Vue的服务器端渲染（SSR）来改善SEO。

# vue 首页加载白屏优化

在Vue应用中，首页加载时出现白屏是常见的问题，因为最初的页面需要在DOM构建完成并加载完所有资源（如CSS、JavaScript、数据）后才能显示内容。以下是一些优化首页加载白屏时间的策略：

懒加载（Lazy Loading）：

对于非首屏的数据，使用懒加载组件（如Vue的<router-view>懒加载）或<img>的src属性的srcset属性，确保只在需要时加载。
预加载（Preloading）：

在用户可能导航的路径上预加载重要的子页面。可以使用vue-router的preload选项设置。
代码分割（Code Splitting）：

使用动态导入（import()函数）将应用分为小型模块，每个模块按需加载。CommonChunks插件和Webpack的Code Splitting相关功能可帮助此处。
优化网络请求：

尽可能压缩CSS和JavaScript文件，减少HTTP请求总数。
使用CDN分发静态资源，减少加载时间。
首屏优化（Prerendering）：

对于SPA，可以使用预渲染技术如PrerenderSPAPlugin或Vue Server Render，生成最初的HTML文件，提高SEO并降低初始加载白屏时间。
最小化白屏时间（DOMContentLoaded）：

尽可能减小初始HTML文件的大小，只加载必要结构和最基本的CSS。
卸载组件（On Demand Loading）：

对于复杂组件，使用计时器延迟或条件加载，如Vue’s provide/inject 功能。
通过组合使用这些优化方法，可以显著减少用户打开首页时白屏时间，提高用户体验。

# css 伪类和伪元素区别

在CSS中，伪类（pseudo-class）和伪元素（pseudo-element）是用于选择或描述HTML元素在特定状态下的不同部分或行为，它们的主要区别在于：

伪类（Pseudo Classes）:

伪类用于选择某些特定状态的元素，比如:hover（当鼠标悬停时）、:active（当元素被激活，例如按钮被点击）或:first-child（第一个子元素）等。
它们通常用于在元素的不同状态或交互过程中定义样式，但并不直接附加到文档结构中。
伪元素（Pseudo Elements）:

伪元素则用于插入到元素内部或外部，如::before（在元素前插入内容）和::after（在元素后插入内容）。
它们是对元素HTML结构的扩展，并且常用于创建符号、图标、装饰性内容或生成文本前后的额外样式。
总结来说，伪类关注元素的行为和状态，而伪元素关注元素的内容或结构增强。在实现某些自定义样式或动态效果时，两者都有其独特的用处。
